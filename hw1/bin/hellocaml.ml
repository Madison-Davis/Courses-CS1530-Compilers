...
(*
  ...
  Please feel ask a course staff member for help if you get stuck. Learning OCaml
  is essential for your successful completion of later projects, but it is
  not supposed to be too onerous in itself!
*)

(*
  If you are using VSCode, please see the instructions on the Resource page
  for how to get started.  In particular, you should start a 
  "sandbox terminal" and then run the command `dune build --watch` while working on 
  this file.
...
(*
   ...
   have `dune build --watch` going at the same time as other work).
...
let cube : int -> int = fun (x:int) -> 
  let cubed = (float_of_int x) ** 3. in
    int_of_float cubed
...
let compose_pair (p : ('b -> 'c) * ('a -> 'b)) : 'a -> 'c =
  let first = match p with | (_, f) -> f in
    let second = match p with | (s, _) -> s in
      (* x is bounded to 'a because the function 'first' has type ('a -> 'b) *)
...
(*
  OCaml has a build-in datatype of generic lists: 
...
let rec list_to_mylist (l : 'a list) : 'a mylist =
  (* tl is represented by [], so we need to check [], 1 entry, or 2+ entries *)
  begin match l with 
  | [] -> Nil
  | h1 :: tl -> Cons (h1, list_to_mylist(tl))
  end
...
let rec rev (l : 'a list) : 'a list =
  let rec reverse result original =
    match original with 
      | [] -> result
      | hd::tl -> reverse (hd :: result) tl
  in
  reverse [] l
...
(*
  ...
  Note that you will need a helper function that takes an extra parameter
...
(* Here, ChatGPT helped me to understand the concept of partial functions and syntax examples *)

let rev_t (l : 'a list) : 'a list =
  let rec tail_recursion acc = function
    | [] -> acc
    | hd::tl -> tail_recursion (hd::acc) tl
  in
  tail_recursion [] l
...
let rec union (l1 : 'a list) (l2 : 'a list) : 'a list =
  ...
  | (l1, []) -> l1
  | ([], l2) -> l2
  | (hd1::tl1, hd2::tl2) -> union (insert hd2 l1) (tl2)
  end
...
(*
  ...
  Hint: you should write some of your own test cases in the file 
        tests/studenttests.ml, to help you get this right. Indeed,
        you should make sure you have appropriate test cases for all of the 
        functions you implement, particular for this problem and later ones.
        If the tests we provide you aren't sufficient, you should write
        your own. We won't mention it again, but note from the handout 
        documentation that a significant number of points are based on 
        the tests that you write. And it's a good habit to get into, 
        for the rest of this course and beyond!
*)

(* Here, ChatGPT helped me to learn syntactically how to write multiple lines after a pattern-match.
   ChatGPT also helped me to find where I was getting an infinite loop *)

let rec vars_of (e : exp) : string list =
  let rec helper_vars_of e acc = 
    begin match e with
    | Const (a) -> acc
    | Var (a) -> union [a] acc
    | Add (a, b) -> 
       let varsa = helper_vars_of a acc in
       let varsb = helper_vars_of b acc in
       union varsa varsb
    | Mult (a, b) -> 
      let varsa = helper_vars_of a acc in
      let varsb = helper_vars_of b acc in
      union varsa varsb
    | Neg (a) -> helper_vars_of a acc
    end
  in
  helper_vars_of e []
...
(* Here, ChatGPT helped me to learn the difference between = and == when I was debugging.
   = means equal as content
   == means equal as physical memory *)
...
let rec interpret (c : ctxt) (e : exp) : int64 =
  let rec helper_interpret e (acc : int64) = 
    begin match e with
    ...
    (* NEGATIVES *)
    | Neg (a) -> 
      let var1 = helper_interpret a acc in
      Int64.neg var1
    
    (* MULTIPLY *)
    | Mult (a, b) ->
      let var1 = helper_interpret a acc in
      let var2 = helper_interpret b acc in
      Int64.mul var1 var2
...
(* Here, ChatGPT helped me to learn how to debug using Printf statements and let () = function to output 
   ChatGPT also helped me to find the place where I was experiencing a potential infinite loop *)

let rec optimize (e : exp) : exp =
  let rec helper_optimize (e :exp) : exp = 
    begin match e with
    (* CONSTANTS/VARIABLES *)
    | Const a -> e
    | Var a -> e
    ...
    | Neg a -> 
      begin match a with
      | Const 0L -> Const 0L
      | Neg a -> helper_optimize a
      | _ -> 
        let var1 = helper_optimize a in
        if var1 = a then e 
        else helper_optimize (Neg var1)
      end
    ...
    | Add (a, b) -> 
      ...
      | (Var a, Var b) -> e
      | (Var a, Const 0L) -> Var a
      | (Const 0L, Var b) -> Var b

      (* const/var cases where there might be nested mult/add/negs *)
      | (a, b) -> 
          let var1 = helper_optimize a in
          let var2 = helper_optimize b in
          if var1 = a && var2 = b then e else
          helper_optimize (Add (var1, var2))
      end
    ...
    | Mult (a, b) -> 
      ...
      | (Var a, Var b) -> e
      | (Const 0L, Var b) -> Const 0L
      | (Var a, Const 0L) -> Const 0L
      | (Const 1L, Var b) -> Var b
      | (Var a, Const 1L) -> Var a

      (* const/var or cases where there might be nested mult/add/negs *)
      | (a, b) -> 
        let var1 = helper_optimize a in
        let var2 = helper_optimize b in
        if var1 = a && var2 = b then e else
        helper_optimize (Mult (var1, var2))
      end
    end
  in
  helper_optimize e
...
let rec compile (e : exp) : program =
  let rec helper_compile e program =
    begin match e with
    | Const a -> append program [IPushC a]
    | Var a -> append program [IPushV a]
    | Mult (a, b) ->
      let program_res1 = helper_compile a program in
      let program_res2 = helper_compile b program_res1 in
      append program_res2 [IMul]
    | Add (a, b) ->
      let program_res1 = helper_compile a program in
      let program_res2 = helper_compile b program_res1 in
      append program_res2 [IAdd]
    | Neg (a) ->
      let program_res1 = helper_compile a program in
      append program_res1 [INeg]
    end
  in
  helper_compile e []
